...
import { RoomSelection, ValidationErrors, FloorTypePercentages } from '../types';
...
interface SqftRoomsStepProps {
  ...
  rooms: RoomSelection[];
  onChange: (data: { sqft: number; floorTypePercentages: FloorTypePercentages; rooms: RoomSelection[] }) => void;
  errors: ValidationErrors;
}

export default function SqftRoomsStep({ sqft, floorTypePercentages, rooms, onChange, errors }: SqftRoomsStepProps) {
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="max-w-sm">
          ...
          {errors.sqft && (
            <p className="text-red-600 text-sm mt-2">{errors.sqft}</p>
          )}
          <p className="text-gray-600 text-sm mt-2">
            Enter the total cleanable square footage
          </p>
        </div>
      </div>
      ...
      {sqft > 0 && (
        <div className="border-t border-gray-200 pt-8">
          <FloorTypeSlider 
            ...
            onChange={updateFloorTypePercentages}
          />
        </div>
      )}
      ...
      <div className="border-t border-gray-200 pt-8">
        ...
        <RoomPicker selectedRooms={rooms} onChange={updateRooms} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white rounded-lg p-4">
              <div className="text-center">
                <div className="text-3xl font-bold text-[#001F54] mb-1">
                  {sqft.toLocaleString()}
                </div>
          ...
          {rooms.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-200">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                {rooms.map((room) => (
                  <div key={room.roomType} className="flex justify-between bg-gray-50 rounded px-3 py-1">
