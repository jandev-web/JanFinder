...
const GoldServiceList: React.FC<PackageProps> = ({ pkg, recPackage }) => {
    const rec = pkg.packageType === recPackage.packageType ? true : false;

    return (
        <div className={`min-w-[240px] relative overflow-hidden max-w-xs bg-white rounded-b-2xl shadow-lg px-6 pb-6 text-center hover:shadow-xl transition-shadow duration-300 ${rec ? 'border-b-4 border-l-4 border-r-4 border-[#001F54]' : ''} flex flex-col justify-between`}>
            <div className="">
                <Accordion>
...
export default GoldServiceList;
