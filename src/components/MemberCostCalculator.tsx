'use client';
import React, { useState, useEffect } from 'react';

interface MemberCostCalculatorProps {
    initialTime: number;
    frequency: any;
}

const MemberCostCalculator: React.FC<MemberCostCalculatorProps> = ({ initialTime, frequency }) => {
    const [profitPercentage, setProfitPercentage] = useState<number>(20);
    const [salary, setSalary] = useState<number>(0);
    const [overhead, setOverhead] = useState<number>(0);
    const [payroll, setPayroll] = useState<number>(0);
    
    const [preProfitCost, setPreProfitCost] = useState<number>(0);
    const [totalCost, setTotalCost] = useState<number>(0);
    const [monthTime, setMonthTime] = useState<number>(0);
    const [dayTime, setDayTime] = useState<number>(initialTime/60);
    
    useEffect(() => {
        const setTimes = () => {
            switch (frequency) {
                case 'None':
                case null:
                    setDayTime(0);
                    setMonthTime(0);
                    break;
                case 'One Time':
                    setDayTime(initialTime/60);
                    setMonthTime(1);
                    break;
                case 'Weekly':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 4.33)/60);
                    break;
                case '2 Days a Week':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 8.66)/60);
                    break;
                case '3 Days a Week':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 13)/60);
                    break;
                case '4 Days a Week':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 17.33)/60);
                    break;
                case '5 Days a Week':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 21.66)/60);
                    break;
                case '6 Days a Week':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 26)/60);
                    break;
                case '7 Days a Week':
                    setDayTime(initialTime/60);
                    setMonthTime((initialTime * 30)/60);
                    break;
                case '1 Day a Month':
                    setDayTime(initialTime/60);
                    setMonthTime(initialTime/60);
                    break;
                case 'Quarterly':
                    setDayTime(initialTime/60);
                    setMonthTime(0);
                    break;
                case 'Yearly':
                    setDayTime(initialTime/60);
                    setMonthTime(0);
                    break;
                default:
                    setDayTime(initialTime/60);
                    setMonthTime(0);
                    break;
            }
            calculatePreProfitCost();
        };

        setTimes();
    }, [frequency, salary, payroll, overhead, initialTime]);

    const calculatePreProfitCost = () => {
        const laborCost = monthTime * salary;
        const finalPayroll = payroll * monthTime;
        const finalOverhead = overhead * monthTime;
        const preProfit = laborCost + finalOverhead + finalPayroll;
        setPreProfitCost(preProfit);
    };

    const calculateCost = () => {
        const finalCost = preProfitCost / (1 - (profitPercentage / 100));
        setTotalCost(finalCost);
    };

    const changeOverhead = (newOverhead: any) => {
        setOverhead(parseFloat(newOverhead))
        calculatePreProfitCost
    }

    const changeSalary = (newSalary: any) => {
        setSalary(parseFloat(newSalary))
        calculatePreProfitCost
    }

    const changePayroll = (newPayroll: any) => {
        setPayroll(parseFloat(newPayroll))
        calculatePreProfitCost
    }

    return (
        <div className="bg-[#001F54] text-white p-10 rounded-lg shadow-xl max-w-lg mx-auto border-2 border-yellow-500">
            <h2 className="text-3xl font-extrabold mb-6 text-yellow-500 text-center">Cost Calculator</h2>
            <div>
                <p>Hours</p>
                <div>
                    <p>{dayTime / 60}/day</p>
                    <p>{monthTime / 60}/mo</p>
                </div>
            </div>
            <p>{salary*monthTime}</p>

            <label className="block text-yellow-500 font-semibold mb-2">Hourly Salary ($)</label>
            <input
                type="number"
                value={salary}
                onChange={(e) => changeSalary(e.target.value)}
                className="w-full p-3 border-2 border-yellow-500 rounded-lg text-[#001F54] focus:outline-none focus:ring-2 focus:ring-yellow-500 mb-4"
            />
            <p>{payroll}</p>
            <label className="block text-yellow-500 font-semibold mb-2">Payroll Tax</label>
            <input
                type="number"
                value={payroll}
                onChange={(e) => changePayroll(e.target.value)}
                className="w-full p-3 border-2 border-yellow-500 rounded-lg text-[#001F54] focus:outline-none focus:ring-2 focus:ring-yellow-500 mb-4"
            />
            <p>{payroll}</p>
            <label className="block text-yellow-500 font-semibold mb-2">Overhead</label>
            <input
                type="number"
                value={payroll}
                onChange={(e) => changeOverhead(e.target.value)}
                className="w-full p-3 border-2 border-yellow-500 rounded-lg text-[#001F54] focus:outline-none focus:ring-2 focus:ring-yellow-500 mb-4"
            />
            <p>Pre-Profit Cost: ${preProfitCost.toFixed(2)}</p>
            <label className="block text-yellow-500 font-semibold mb-2">Profit Percentage (%)</label>
            <input
                type="number"
                value={profitPercentage}
                onChange={(e) => setProfitPercentage(parseFloat(e.target.value))}
                className="w-full p-3 border-2 border-yellow-500 rounded-lg text-[#001F54] focus:outline-none focus:ring-2 focus:ring-yellow-500 mb-4"
            />

            <button
                onClick={calculateCost}
                className="w-full py-2 bg-yellow-500 text-[#001F54] font-bold rounded-lg hover:bg-yellow-400 transition-all mb-4"
            >
                Calculate Cost
            </button>

            <p className="text-yellow-500 font-semibold text-lg">Total Cost: <span className="text-white">${totalCost.toFixed(2)}</span></p>
        </div>
    );
};

export default MemberCostCalculator;
