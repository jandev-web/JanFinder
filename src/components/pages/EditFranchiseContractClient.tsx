// components/pages/FranchiseEditContractClient.tsx
'use client';

import React, { useMemo, useState } from 'react';
import { useRouter } from 'next/navigation';
import { FileUploader } from '@aws-amplify/ui-react-storage';
import getContractTemplate from '@/utils/getContractTemplateClient';
import testFranchiseContractTemplate from '@/utils/testFranchiseContractTemplateClient';
...
export default function FranchiseEditContractClient({ owner, franchise, setTemplate, deleteTemplateAndUnset }: Props) {
  ...
  const hasTemplate = !!(
    franchise?.data?.contractTemplate &&
    franchise?.data?.contractTemplate !== 'none' &&
    franchise?.data?.contractTemplate !== false
  );

  const currentTemplateName =
    typeof franchise?.contractTemplate === 'string'
  ...
  const downloadContractTemplate = async () => {
    ...
    setBusy(true);
    try {
      const tmpl = await getContractTemplate(franchiseID);
      if (!tmpl?.url) throw new Error('No URL returned for contract template');
      ...
      setError('Failed to download contract template. Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto bg-white shadow-md rounded-xl p-10 relative">
      ...
      <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">Edit Franchise Contract</h2>
      ...
      <div className="mb-8">
        ...
        {hasTemplate && (
          ...
          >
            {deleteBusy ? 'Deleting Template...' : 'Delete Contract Template'}
          </button>
        )}
      </div>

      <div className="flex flex-col gap-6">
        <h3 className="text-xl font-semibold">Upload Franchise Contract Template</h3>

        <FileUploader
          acceptedFileTypes={['.docx']}
          path={`members/franchise/${franchiseID}/templates/contract/`}
          maxFileCount={1}
          isResumable={false}
          // Always upload to the expected key; we'll roll back (delete + skip DDB) on failure.
          processFile={({ file }) => ({ file, key: 'contract-template.docx' })}
          onUploadStart={() => {
            setError(null);
            setIssues([]);
            setTesting(false);
          ...
          onUploadSuccess={async () => {
            try {
              ...
              setTesting(true);
              const res = await testFranchiseContractTemplate(franchiseID);
              setTesting(false);
              ...
              await setTemplate(franchiseID, 'contract', true);

              // 4) Show warnings (if any) and link to test PDF
              setIssues(allIssues);
              setTestPdfUrl(typeof payload?.pdfUrl === 'string' ? payload.pdfUrl : null);

              // 5) Refresh so "Current Contract Template" link appears if it was missing
              router.refresh();
            } catch (e: any) {
