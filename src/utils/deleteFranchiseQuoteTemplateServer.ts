...
export default async function deleteFranchiseQuoteTemplateServer(franchiseID: string) {
  const client = createServerDataClient(cookies);
  const res = await client.mutations.deleteFranchiseQuoteTemplate({ franchiseID }, { authMode: 'userPool' });
  ...
  if (data && typeof data === 'object' && 'statusCode' in data && 'body' in data) {
    ...
    if (status !== 200) throw new Error(`delete quote template failed: ${bodyRaw}`);
