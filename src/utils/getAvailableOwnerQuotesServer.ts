// src/utils/getAvailableQuotesServer.ts
import 'server-only';
import { cookies } from 'next/headers';
import { createServerDataClient } from '@/utils/data-server';

// Keep the param optional so existing call sites don't break, but it's unused.
export async function getAvailableQuotesServer(_ownerId?: string): Promise<any[]> {
  const client = createServerDataClient(cookies);

  const res = await client.queries.getAvailableQuotesOwner(
    { authMode: 'userPool' } 
  );
  ...
  if (data && typeof data === 'object' && 'statusCode' in data && 'body' in data) {
    ...
    if (status !== 200) throw new Error(`getAvailableQuotesOwner failed: ${bodyRaw}`);
