// amplify/functions/delete-franchise-quote-template/handler.ts
...
export const handler = async (event: any) => {
  ...
  const base = `members/franchise/${franchiseID}/templates/quote`;
  const keys = [
    `${base}/quote-template.docx`,
    `${base}/quote-template-test.docx`,
    `${base}/quote-template-test.pdf`,
  ];
  ...
  await ddb.send(
    new UpdateItemCommand({
      ...
      ExpressionAttributeNames: { '#qt': 'quoteTemplate' },
